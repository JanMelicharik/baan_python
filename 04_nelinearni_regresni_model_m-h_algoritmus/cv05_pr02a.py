...
# Kandidatska hustota N(mu, d**2)
d = 10
mu = 0

# ===== 2. Independence chain M-H algoritmus =====

print("Independence chain M-H algoritmus:")
for i in range(1, s):
    # a) Generovani kandidata theta_ast
    theta_ast = np.random.normal() * d + mu
    # b) Spocitani akceptacni pravdepodobnosti
    alpha = min(math.exp(-1/2 * abs(theta_ast) + 1/2 * abs(theta[i-1]) - 1/(2 * d**2) * (theta[i-1] - mu)**2 + 1/(2 * d**2) * (theta_ast - mu)**2), 1)
